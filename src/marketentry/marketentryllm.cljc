(ns marketentry.marketentryllm
  ...
  the TUR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-tr-entity?=" (:has-tr-entity? e)
                        " vkn-verified?=" (:vkn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-tr-entity? e))
                              (:has-tr-entity? e))
                          (or (not (:requires-vkn? e))
                              (:vkn-verified? e)))
                   0.9 0.3)}))
