    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(VKN/統一資格/EKAP登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- tr-entity-missing-violations
...
(defn- vkn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-vkn? true`, INDEPENDENTLY check
  `:vkn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-vkn? e))
                 (not (true? (:vkn-verified? e))))
        [{:rule :vkn-unverified
          :detail (str subject " はVKN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (tr-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (vkn-unverified-violations request st)
